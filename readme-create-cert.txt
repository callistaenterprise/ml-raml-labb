...
openssl req -verbose -new -x509 -key private/ca.key -passin pass:password -out public/ca.crt -days 7300 -set_serial 1234 -subj "/CN=localhost/OU=NOT FOR PRODUCTION/O=TEST/ST=No-state/C=SE"
...
openssl req -new -key private/server.key -passin pass:password -out server.csr -days 7300 -subj "/CN=localhost/OU=NOT FOR PRODUCTION/O=TEST/ST=No-state/C=SE"
...
openssl req -new -newkey rsa:1024 -nodes -passout pass:password -out client/client.req -keyout client/client.key  -days 7300 -subj "/CN=localhost/OU=NOT FOR PRODUCTION/O=TEST/ST=No-state/C=SE"
...
keytool -genkey -alias dummy -keyalg RSA -keystore truststore.jks -storepass password -keypass password -dname "CN=localhost, OU=NOT FOR PRODUCTION, O=TEST, ST=No-state, C=SE"
